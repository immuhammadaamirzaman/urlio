...
are tracked in Redis (a per-user set plus one key each, holding session metadata) so
logout and logout-all can revoke them and a replayed/rotated token is rejected. Access
tokens are bound to their session (``sid`` = refresh jti) so revoking a session
immediately invalidates its access tokens too.
"""
